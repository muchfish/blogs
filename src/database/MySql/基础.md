...
```mermaid
flowchart TD 
  subgraph "1.识别出所有的并列节点、切换节点"
    A1[1.1 找出该spd下所有回路] --> A2[1.2 过滤没有回路的数据]
    A2 --> A3[1.3 找出回路分析]
    subgraph "1.3 回路分析"
      A3.1[1.3.1 查询回路]
      A3.2[1.3.2 分析回路中的节点]
      subgraph "1.3.2 节点分析"
        A3.2.1[1.3.2.1 查询回路节点]
        A3.2.2[1.3.2.2 所有节点成对出现]
        A3.2.3[1.3.2.3 继电器切换]
      end
    end
    A3 --> A4[1.4 查找切换节点]
    subgraph "1.4 查找切换节点"
  ...
  subgraph "2. 获取起点终点并按回路类型分组"
    B1[2.1 找出该spd下所有回路] --> B2[2.2 找出回路分析]
    subgraph "2.2 回路分析"
      B2.1[2.2.1 查询回路]
      B2.2[2.2.2 查询回路节点]
      subgraph "2.2.2 回路节点分析"
        B2.2.1[2.2.2.1 分析黑屏柜]
        B2.2.2[2.2.2.2 查询起点所属的cable]
        B2.2.3[2.2.2.3 分析出abc三项]
        B2.2.4[2.2.2.4 分析回路类型]
      end
    end
    B2 --> B3[2.3 按回路类型分组]
  end

  subgraph "3. 组装交流电压大图数据"
    C1[3.1 按同一个N相组合abc三项]
    C1 --> C2[3.2 组装大图]
  end

  subgraph "4. 组装电压切换大图数据"
    D1[4.1 按同一个N相组合abc三项]
    D1 --> D2[4.2 将普通的回路筛选出来组成普通大图]
    D2 --> D3[4.3 按切换关系分组]
    subgraph "4.3 按切换关系分组"
      D3.1[4.3.1 按切换关系分组]
      D3.2[4.3.2 分析切换关系]
    end
    D3 --> D4[4.4 按切换节点直接组成大图]
    subgraph "4.4 切换节点组成大图"
      D4.1[4.4.1 用切换屏柜加上切换装置分组]
      D4.2[4.4.2 按同一个N相组合abc三项]
      D4.3[4.4.3 切换屏柜]
    end
  end
  ...
  subgraph "6. 组装切换并列大图数据"
    F1[6.1 分析并列关系]
    F1 --> F2[6.2 按同一个N相组合abc三项]
    F2 --> F3[6.3 按切换关系分组]
    F3 --> F4[6.4 组装切换大图]
  end

  G[7. 保存大图数据，建立电缆芯关联关系]
  H[8. 设置电缆芯映射关系]
  I[9. 设置屏柜映射关系]
  J[10. 结束]
...
这个流程图描述了电气行业中组装交流电压大图数据的执行逻辑，包含从识别并列节点和切换节点开始，到最终保存大图数据和结束的各个步骤。